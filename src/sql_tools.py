...
class StDbAccess():
    ...
    def connect(self):
        ...
        self.conn.execute(
            "CREATE TABLE IF NOT EXISTS server_data (guild_id numeric unique, settings text, b0 text, b1 text, b2 text, b3 text, b4 text)")
    ...
    def get1(self, id: str):
        item = self.conn.execute(
            f'SELECT settings FROM server_data WHERE guild_id = ?', (id,)).fetchone()
        self.close()
        if item:
            if item[0]:
    ...
    def get2(self, id: str):
        ...
        for column in columns:
            item = self.conn.execute(
                f'SELECT {column} FROM server_data WHERE guild_id = ?', (id,)).fetchone()
            if item:
                if item[0]:
    ...
    def set1(self, id: str, obj):
        self.conn.execute(
            'INSERT OR IGNORE INTO server_data (guild_id) VALUES (?)', (id,))
        self.conn.execute(
            'UPDATE server_data SET settings=? WHERE guild_id=?', (obj, id))
        self.close()

    def set2(self, id: str, obj, index: int = 0):
        self.conn.execute(f'UPDATE server_data SET b{
                          index}=? WHERE guild_id=?', (obj, id))
        self.close()
...
def isLuigi(guildID: int) -> bool:
    global db
    db.connect()
    settings = db.get1(str(guildID))
    if settings:
