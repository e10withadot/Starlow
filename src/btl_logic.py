            ...
            for guy in lineup:
                ...
                while one_more:
                    ...
                    if guy.__class__.__name__ == "Player":
                        ...
                        emojis = ['<:st_jump:1136186635839623268>',
                                  '<:st_hammer:1136186760041336893>']
                        # enemy info
                        etext = ""
                        for i in eIndex:
                            enemy = lineup[i]
                        ...
                        if guy.stickers:
                            desc += f"\n\n> Inventory({guy.maxFP -
                                                       guy.FP}/{guy.maxFP}): {icons}"
                        embed = (
                            c.StEmbed(
                                title=f"{guy.name}'s Turn!", description=desc)
                            .set_footer(f"{self.settings['coins']} coins")
                        )
                        ...
                        if one.id == 0 or one.id == 1:
                            ...
                            # get basic move path
                            path = os.path.abspath(__file__).replace(
                                r"\btl_logic.py", r"\templates\basic.json")
                            with open(path) as f:
                        ...
                        # sticker album
                            ...
                            # set sticker prompt
                            desc = f"{guy.maxFP -
                                      guy.FP}/{guy.maxFP} stickers.\n"
                            emojis = []
                            for id, num in guy.stickers.items():
                                desc += f"\n**{guy.moves[id].icon} {
                                    guy.moves['names'].icon} x{num}** | {guy.moves[id].info}"
                        ...
                        for guy in lineup:
                            if hasattr(guy, "id"):
                                if guy.id == int(nums[0])-1:
                                    # the selected stat
                                    stat = int(getattr(guy, re.findall(
                                        r'\D+$', condition)[0].upper()))
                                    break
                                else:
                                    stat = None
                        if stat:
            ...
            if met:
                ...
                for result in results:
                    ...
                    # execute move
                        ...
                        data = re.findall(
                            r'(?i:(?<=^move:)).*', wResult)[0].split(',')
                        if len(data) == 2:
                            # format: [enemy index],[move index]
                            info = self.enemies[int(
                                data[0])].moves[int(data[1])]
                            name = info.name
                            # set move target
                            target = pIndex
                        else:
                            ...
                            info['hits'] = data[1]
                            info['type'] = [
                                "Ground", "Aerial", "Magic"][data[2]]
                            info['offense'] = bool(data[3])
                            info['stat'] = ["HP", "FP", "POW",
                                            "DEF", "SPEED", "STACHE"][data[4]]
                    ...
                    # dialogue events
                        ...
                        webhook = None
                        event = self.dialogue["events"][int(
                            re.findall(r'\d', wResult)[0])-1]
                        for line in event:
                            if not webhook:
                    ...
                    # spawn enemies
                        ...
                        spawns = re.findall(r'(?i:(?<=^spawn:))\d,\d', result)[
                            0].split("&")
                        for spawn in spawns:
                            i = spawn.split(",")
